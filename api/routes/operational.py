...
@router.post("/ppm")
def ppm(body: dict, principal: Principal = Depends(require_operator)):
    return get_registry().run("ppm.resolve", body, actor=principal.actor, role=principal.role)


@router.post("/connectors/{name}/ingest")
def ingest(name: str, body: dict, _: Principal = Depends(require_operator)):
...
def vision(name: str, _: Principal = Depends(require_operator)):
